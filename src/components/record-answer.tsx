...
const saveUserAnswer = async() => {
  ...
  useEffect(() => {
    // Update current speech in real-time
    if (results && results.length > 0) {
      const latestResult = results[results.length - 1];
      if (typeof latestResult !== 'string') {
        const newTranscript = latestResult.transcript;
        setCurrentSpeech(newTranscript);
        setUserAnswer(prevAnswer => {
          return prevAnswer ? `${prevAnswer} ${newTranscript}` : newTranscript;
        });
      }
    }
  }, [results]);
      ...
      <div className="w-full mt-4 p-4 border rounded-md bg-gray-50">
        ...
        {currentSpeech && (
          <p className="text-sm text-gray-500 mt-2">
            <strong>Current Speech:</strong> {currentSpeech}
          </p>
        )}
      </div>
    </div>
